...
1. For each of the *factors* under consideration, we will need to stack up STM scores from 4 tests, i.e. four columns. Since we have three factors, we will get three sets of stacked scores, each from a (shuffled) set of 4 tests.
1. We will set up these stacked scores in Col A on three separate sheets, similar to the one shown, titled "Permutation Test".
1. Create Histograms for each of the columns using WTFcsv. Ensure that there are *TWO* Histograms: one for each value of the factor under consideration. 
