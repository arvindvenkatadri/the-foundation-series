...
1. Let us first examine the street traffic data we have gathered for any "model-like" pattern!
...
2. Let us throw some chalk or Lego pieces on the floor at random and see what happens!!
...
1. Do you remember plucking mangoes from the trees in your childhood gardens?  
1. Did you happen to be the youngest in the group? 
1. Did you have run home to get the salt + chilly powder? 
1. Why did you trust your friends not to eat off the best mangoes in your absence? ;-D
1. Were you used to being the "*denner*" in every game?
1. Was that a completely "mad" thing to do, throwing stuff on the floor? Or was it an act of *Pedagogical Vandalism*?
1. Is Randomness just too much to handle or comprehend?
1. Or can we "tame" it in some way, and put it to use in Art, Design, and Tech?
1. Have we been doing this all along? Have we been "*holding infinity in the palm of our hands*"[^1], in the हाथ कीे लकीरे?
1. What diagrams or thought experiments can we create to do this?
1. WHY would this be a good idea? Is there a very **Human Motivation** behind this desire we might have?
...
Did Hamlet have an idea, any suspicion, that he might have wanted to prove? Yes, he did, and that is why he *feigned madness*, so people would open up and tell him things, in an unguarded way. So there was [*craft in his madness*](http://www.shakespeare-online.com/plays/hamlet/antichamlet.html).

If we wanted to know the truth of something, some guess, or a **Hypothesis** that we had, then we would need to conduct an "Experiment", rather like Hamlet did, without quite going or **feigning madness** of course. But there is a **Madness to the Method**, as we will see...
...
Let us consider the **Short Term Memory** of Foundation Students:   

- Is it different between those who pursue "*Art-related*" degrees and those who pursue "*Design-related*" degrees?   
- Between Male and Female students?   
- Any other factors, not related to types of individuals, but more like parameters of STM itself?  

Or we could use the [**Guilford Test for Alternative Uses**](https://curtbonk.com/bobweb/r546/modules/creativity/creativity_tests/guilford_uses_task.html) as our vehicle for testing similar Hypotheses. 
...
Once our test is complete, let us collate all the data for all tests using Excel. (This is for the STM Hypothesis; we can adapt this spreadsheet for any other hypothesis test with categorical factors and scores.)

1. Download this [**spreadsheet**](STM.xlsx) and save as your own *named* local copy. 
1. Enter the data from all tests on the first sheet titled "RawData" (the one with the warning)
1. **Duplicate this data** on the sheet titled "Data Duplicated". **Do not touch** the "RawData" sheet again. 
1. For each of the *factors* under consideration, we will need to stack up STM scores from $2^3 = 8$ tests, i.e. eight columns. 
1. If we have three two-valued, categorical factors, we will get three sets of stacked scores, each from a (shuffled) set of the  tests. 
1. Half the scores in these stacked columns will pertain to one *level* of the factor, and the other half of the scores will pertain to the other level. (This should remind you of the Karnaugh Map you may have learnt in your digital logic courses in school.). See example below: it shows how we can stack up the scores from *4* tests for each *level* of the parameter *p1*.
...
1. Create *paired* Histograms for each of the two stacked-score columns using WTFcsv, or Excel itself if you are confident. Ensure that there are *TWO* Histograms: one for each value of the factor under consideration. 
1. Inspection of the shapes and locations of these paired Histograms may give you an idea whether the factor under consideration has any effect on STM scores or none. 
1. Save these 6 histograms as PNG files on your machines and use them along with *Comic Generator* (discussed below) to tell the story of your Hypothesis Testing. 
...
### Data Analysis-2: Permutation Tests using Excel
...
1. You will need to replicate that for the other two factors.



### References

1.   Seeing Probability Theory: A Superb **visual** website to see patterns in theory! Go and play there with Hamlet! [**(Web Link)**](https://seeing-theory.brown.edu/basic-probability/index.html)
...
1. Tim C. Hesterberg (2015) *What Teachers Should Know About the Bootstrap: Resampling in the Undergraduate Statistics Curriculum*, The American Statistician, 69:4, 371-386, DOI: 10.1080/00031305.2015.1089789. Available Online [**(Web Link)**](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4784504/)

1. Karan Kashyap, *Calculating the Value of Pi: A Monte Carlo Simulation*, <https://www.cantorsparadise.com/calculating-the-value-of-pi-using-random-numbers-a-monte-carlo-simulation-d4b80dc12bdf>

1. Mahashweta Devi, *The Why Why Girl*, <https://www.tulikabooks.com/picture-books/the-why-why-girl-english.html>, read by Sarah Dryden Peterson <https://www.youtube.com/watch?v=xNcV5DZhg4c>
...
To be or Not to be, in [Tok Pisin](https://www.hawaii.edu/satocenter/langnet/definitions/tokpisin.html):
...
[^1]:William Blake, *Auguries of Innocence*, https://www.poetryfoundation.org/poems/43650/auguries-of-innocence
