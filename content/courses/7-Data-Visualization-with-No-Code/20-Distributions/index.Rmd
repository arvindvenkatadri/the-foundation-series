...
``` 
```{r}
banned_by_state <- banned %>% group_by(State) %>% summarise(total = n())
banned %>% group_by(State, `Type of Ban`) %>% summarise(count = n()) %>%
...
ggplot(aes(x = reorder(State, banned_by_state$total), y = count, fill = `Type of Ban`)) + geom_col() + coord_flip()
```
