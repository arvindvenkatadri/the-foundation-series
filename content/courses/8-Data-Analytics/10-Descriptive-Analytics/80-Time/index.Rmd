...
This material has been drawn largely from Andrew Heiss' tutorial on Time Series Analysis.
...
Here we define a bunch of rolling mean functions. These don't do anything yet—once you feed them some data, they'll calculate the mean of the past number of periods given in `window` argument. 
...
ggplot(births_monthly_long, aes(x = date, y = avg_births)) +
  ...
  geom_line(aes(y = value, color = window_size), linewidth = 0.75) + 
  theme_minimal()
...
Now we can decompose this with the `stats::stl()` function (which stands for “seasonal, trend, and irregular, with loess”). `s.window` is set to periodic because the help file and every example I found online said to do that.
