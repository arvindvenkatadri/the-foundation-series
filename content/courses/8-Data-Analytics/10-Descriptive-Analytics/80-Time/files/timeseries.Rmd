...
library(tidyverse) # Processing and Plotting Tidy Data
library(mosaic) # Our trusted tool
...
There are other plot packages to investigate,
such as [dygraphs](https://rstudio.github.io/dygraphs/index.html)

Recall that we have used the `tsibble` format for the data.
There are other formats such as `ts`, `xts` and others which are meant
for time series analysis. But for our present purposes, we are able to
do things with the capabilities of `timetk`.
